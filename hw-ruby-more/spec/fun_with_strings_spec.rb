...
describe 'anagram grouping' do 
  ...
  it 'for "monkey onkeym for ofr four test ttes"' do
    @anagrams =  'monkey onkeym for ofr four test ttes'.anagram_groups
    @anagrams.each { |group| group.sort! }
    [%w(monkey onkeym), %w(four), %w(for ofr), %w(test ttes)].each do |group|
        expect(@anagrams).to include(group)
    end
  end
